...
function BookDetailPagesPage({dispatch, bookDetailPages, dialog}: BookDetailPagesPagePropsType) {
  const {pages, page, pageSize, count} = bookDetailPages;
  const onPageChange = (toPage: number, toPageSize: number = 20) => {
    dispatch({
      type: "bookDetailPages/setPage",
      payload: {
        page: toPage,
        pageSize: toPageSize
      }
    ...
    dispatch({
      type: "bookDetailPages/queryPages",
    })
  };
  ...
  return (
    <div className={styles.main}>
      ...
      <PageCollection pages={pages} onSetOrderActionClick={onSetOrderClick} onSetCover={onCropCoverDialogOpen}/>
      <CoverCropDialog
        ...
        imgURL={croverURL}
      />
      <PageOrderDialog isOpen={Boolean(dialogs[setOrderDialogKey])} onCancel={onSetOrderDialogCancel}
...
export default connect(({bookDetailPages, dialog}: ConnectState) => ({bookDetailPages, dialog}))(BookDetailPagesPage);
