...
const BookDetailPageModel: BookDetailPageModelType = {
  ...
  effects: {
    * queryPages(_, {call, put, select}) {
      ...
      queryTagsResponse.result.forEach((pageItem: Page) => pageItem.path = `${ApplicationConfig.apiURL}${pageItem.path}`);
      yield put({
  ...
  reducers: {
    ...
    setPageOrder(state, {payload: {id, order}}) {
      ...
      (pages.filter((page:Page) => page.id !== id)).forEach((page:Page) => {
