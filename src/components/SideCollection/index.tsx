...
const SideCollection = ({ dispatch, dialog, sideCollection }: SideCollectionPropsType) => {
  ...
  return (
    ...
    >
      <Tabs activeKey={sideCollection.activeTab} onChange={onActiveChange} className={styles.tabs}>
      ...
      <div className={styles.actionBar}>{renderActionBar()}</div>
    </Drawer>
  );
};
export default connect(({ dialog, sideCollection }: ConnectState) => ({ dialog, sideCollection }))(
  SideCollection,
);
